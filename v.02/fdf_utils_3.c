...
/*   Updated: 2022/03/12 08:56:41 by pojea-lo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_line_ppx(t_mlx *gr, t_num *num, t_str *str)
{
	...
	l.i = -1;
	printf ("num->xx: %f y num->y:%f\n", num->xx, num->yy);
	l.xxx = num->xx;
	l.yyy = num->yy;
	printf ("lxx: %f y lyy:%f\n", l.xxx, l.yyy);
	while (++l.i < num->x)
	{
		l.j = -1;
		l.k = 0;
...
void	ft_print_line_3(t_mlx *gr, t_num *l, t_num *n)
{
	...
	{
		gr->addr = mlx_get_data_addr(gr->img, &gr->bits_per_pixel, \
		&gr->line_length, &gr->endian);
		dst = gr->addr + (n->y * gr->line_length + n->x * \
		(gr->bits_per_pixel / 8));
		*(unsigned int *)dst = color;
		mlx_pixel_put(gr->mlx, gr->mlx_win, n->x, n->y, 0xFF0000);
		if (n->av >= 0)
		{
			n->x = n->x + n->IncXi;
			n->y = n->y + n->IncYi;
