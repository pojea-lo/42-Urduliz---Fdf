...
/*   Updated: 2022/03/15 07:44:33 by pojea-lo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	*ft_wd_size(size_t x, size_t y, double *mm)//función que saca el ancho y alto necesario de ventana minima y su centro
{
	...
	if (size == NULL)
		return (0);
	size[0] = 1.3 * (round ((x + y) * cos (0.523599) * cos (0.523599)));//ancho minimo de ventana necesario
	size[1] = 1.3 * (round (((x + y) * sin (0.523599) * cos (0.523599)) + (mm[0] + (-mm[1]))));//alto minimo de ventana necesario
	return (size);
}
...
double	ft_wde_size(double *size)//funcion que saca el valor de la escala en función del size min necesario y una ventana estándar de 900x450
{
	...
	if (escx < escy)
		esc = escy;
	if (esc >= 1 && esc < 3)//para el campo de marte
		esc = 4;
	if (esc < 1)//para el campo de julia
		esc = 2;
	return (esc);
}
